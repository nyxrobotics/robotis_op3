...
void WalkingModule::queueThread()
{
  ...
  ros::ServiceServer get_walking_param_server =
      ros_node.advertiseService("/robotis/walking/get_params", &WalkingModule::getWalkigParameterCallback, this);

  /* sensor topic subscribe */
  ros::Subscriber walking_command_sub =
      ros_node.subscribe("/robotis/walking/command", 0, &WalkingModule::walkingCommandCallback, this);
  ros::Subscriber walking_param_sub =
      ros_node.subscribe("/robotis/walking/set_params", 0, &WalkingModule::walkingParameterCallback, this);
...
void WalkingModule::updateTimeParam()
{
  ...
  l_ssp_start_time_ = (1 - ssp_ratio_) * period_time_ / 4.0;
  l_ssp_end_time_ = (1 + ssp_ratio_) * period_time_ / 4.0;
  r_ssp_start_time_ = (3 - ssp_ratio_) * period_time_ / 4.0;
  r_ssp_end_time_ = (3 + ssp_ratio_) * period_time_ / 4.0;
...
{
  foot_x_offset_ = walking_param_.init_x_offset;
  foot_y_offset_ = walking_param_.init_y_offset;
  foot_z_offset_ = walking_param_.init_z_offset;
  foot_roll_offset_ = walking_param_.init_roll_offset;
  foot_pitch_offset_ = walking_param_.init_pitch_offset;
  foot_yaw_offset_ = walking_param_.init_yaw_offset;
...
void WalkingModule::processPhase(const double& time_unit)
{
  ...
  else if (time_ >= (phase1_time_ - time_unit / 2.0) && time_ < (phase1_time_ + time_unit / 2.0))  // the position of
                                                                                                   // left foot is the
                                                                                                   // highest.
  {
    updateMovementParam();
  ...
  else if (time_ >= (phase2_time_ - time_unit / 2.0) && time_ < (phase2_time_ + time_unit / 2.0))  // middle of double
                                                                                                   // support state
  {
    updateTimeParam();
  ...
  else if (time_ >= (phase3_time_ - time_unit / 2.0) && time_ < (phase3_time_ + time_unit / 2.0))  // the position of
                                                                                                   // right foot is the
                                                                                                   // highest.
  {
    updateMovementParam();
...
bool WalkingModule::computeLegAngle(double* leg_angle)
{
  ...
  if (time_ <= l_ssp_start_time_)
  {
    ...
    right_leg_move.z = wSin(r_ssp_start_time_, z_move_period_time_,
                            z_move_phase_shift_ + 2.0 * M_PI / z_move_period_time_ * r_ssp_start_time_,
                            z_move_amplitude_, z_move_amplitude_shift_);
  ...
  else if (time_ <= l_ssp_end_time_)
  {
    left_leg_move.x =
        wSin(time_, x_move_period_time_, x_move_phase_shift_ + 2 * M_PI / x_move_period_time_ * l_ssp_start_time_,
             x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y =
        wSin(time_, y_move_period_time_, y_move_phase_shift_ + 2.0 * M_PI / y_move_period_time_ * l_ssp_start_time_,
             y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z =
        wSin(time_, z_move_period_time_, z_move_phase_shift_ + 2.0 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw =
        wSin(time_, a_move_period_time_, a_move_phase_shift_ + 2.0 * M_PI / a_move_period_time_ * l_ssp_start_time_,
             a_move_amplitude_, a_move_amplitude_shift_);
    right_leg_move.x =
        wSin(time_, x_move_period_time_, x_move_phase_shift_ + 2.0 * M_PI / x_move_period_time_ * l_ssp_start_time_,
             -x_move_amplitude_, -x_move_amplitude_shift_);
    right_leg_move.y =
        wSin(time_, y_move_period_time_, y_move_phase_shift_ + 2.0 * M_PI / y_move_period_time_ * l_ssp_start_time_,
             -y_move_amplitude_, -y_move_amplitude_shift_);
    ...
    pelvis_offset_l =
        wSin(time_, z_move_period_time_, z_move_phase_shift_ + 2.0 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             pelvis_swing_ / 2.0, pelvis_swing_ / 2.0);
    pelvis_offset_r =
        wSin(time_, z_move_period_time_, z_move_phase_shift_ + 2.0 * M_PI / z_move_period_time_ * l_ssp_start_time_,
             -pelvis_offset_ / 2.0, -pelvis_offset_ / 2.0);
  }
  ...
  else if (time_ <= r_ssp_end_time_)
  {
    left_leg_move.x = wSin(time_, x_move_period_time_,
                           x_move_phase_shift_ + 2.0 * M_PI / x_move_period_time_ * r_ssp_start_time_ + M_PI,
                           x_move_amplitude_, x_move_amplitude_shift_);
    left_leg_move.y = wSin(time_, y_move_period_time_,
                           y_move_phase_shift_ + 2.0 * M_PI / y_move_period_time_ * r_ssp_start_time_ + M_PI,
                           y_move_amplitude_, y_move_amplitude_shift_);
    left_leg_move.z = wSin(l_ssp_end_time_, z_move_period_time_,
                           z_move_phase_shift_ + 2.0 * M_PI / z_move_period_time_ * l_ssp_start_time_,
                           z_move_amplitude_, z_move_amplitude_shift_);
    left_leg_move.yaw = wSin(time_, a_move_period_time_,
                             a_move_phase_shift_ + 2.0 * M_PI / a_move_period_time_ * r_ssp_start_time_ + M_PI,
                             a_move_amplitude_, a_move_amplitude_shift_);
  ...
  // mm, rad
  ep[0] = swap.x + right_leg_move.x + foot_x_offset_;
  ep[1] = swap.y + right_leg_move.y - foot_y_offset_ / 2.0;
  ep[2] = swap.z + right_leg_move.z + foot_z_offset_ - leg_length;
  ep[3] = swap.roll + right_leg_move.roll - foot_roll_offset_ / 2.0;
  ep[4] = swap.pitch + right_leg_move.pitch + foot_pitch_offset_;
  ep[5] = swap.yaw + right_leg_move.yaw - foot_yaw_offset_ / 2.0;
  ep[6] = swap.x + left_leg_move.x + foot_x_offset_;
  ep[7] = swap.y + left_leg_move.y + foot_y_offset_ / 2.0;
  ep[8] = swap.z + left_leg_move.z + foot_z_offset_ - leg_length;
  ep[9] = swap.roll + left_leg_move.roll + foot_roll_offset_ / 2.0;
  ep[10] = swap.pitch + left_leg_move.pitch + foot_pitch_offset_;
  ep[11] = swap.yaw + left_leg_move.yaw + foot_yaw_offset_ / 2.0;
...
void WalkingModule::sensoryFeedback(const double& rlGyroErr, const double& fbGyroErr, double* balance_angle)
{
  ...
  balance_angle[joint_table_["r_hip_roll"]] = op3_kd_->getJointDirection("r_hip_roll") * internal_gain * rlGyroErr *
                                              walking_param_.balance_hip_roll_gain;  // R_HIP_ROLL
  balance_angle[joint_table_["l_hip_roll"]] = op3_kd_->getJointDirection("l_hip_roll") * internal_gain * rlGyroErr *
                                              walking_param_.balance_hip_roll_gain;  // L_HIP_ROLL

  balance_angle[joint_table_["r_knee"]] =
      -op3_kd_->getJointDirection("r_knee") * internal_gain * fbGyroErr * walking_param_.balance_knee_gain;  // R_KNEE
  balance_angle[joint_table_["l_knee"]] =
      -op3_kd_->getJointDirection("l_knee") * internal_gain * fbGyroErr * walking_param_.balance_knee_gain;  // L_KNEE

  balance_angle[joint_table_["r_ank_pitch"]] = -op3_kd_->getJointDirection("r_ank_pitch") * internal_gain * fbGyroErr *
                                               walking_param_.balance_ankle_pitch_gain;  // R_ANKLE_PITCH
  balance_angle[joint_table_["l_ank_pitch"]] = -op3_kd_->getJointDirection("l_ank_pitch") * internal_gain * fbGyroErr *
                                               walking_param_.balance_ankle_pitch_gain;  // L_ANKLE_PITCH

  balance_angle[joint_table_["r_ank_roll"]] = -op3_kd_->getJointDirection("r_ank_roll") * internal_gain * rlGyroErr *
                                              walking_param_.balance_ankle_roll_gain;  // R_ANKLE_ROLL
  balance_angle[joint_table_["l_ank_roll"]] = -op3_kd_->getJointDirection("l_ank_roll") * internal_gain * rlGyroErr *
